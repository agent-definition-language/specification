...
So that `core/0.2.0-draft/spec.md` stays **programmatically transformable** into body-specific output (IETF RFC, ISO, Linux Foundation), follow these conventions when editing the spec.
...
- When adding or renumbering a section, update `core/0.2.0-draft/spec-manifest.yaml` so the `sections` list and optional `subsections` match the spec. The manifest is the source of section order and IDs for generation.
