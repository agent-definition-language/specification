...
**The pattern:** an agent acting on a user's behalf calls other agents and services, and each hop must independently authenticate the caller and authorize the specific operation. This is the core capability ADL adds on top of OAuth 2.1: human-to-agent boundaries authenticate with OAuth credentials ([§10.3.3](/spec/next#1033-credential-schemes)), agent-to-agent boundaries authenticate with passports ([§10.3.1](/protocol#1031-passport-verification-procedure)) and presentation proofs ([§10.3.2](/protocol#1032-presentation-proof)), and authorization scopes ([§10.4](/spec/next#104-authorization-scopes)) compose across the chain so that no single hop sees more authority than it needs.
...
This walkthrough traces every authentication and authorization decision along the way, with explicit citations to the corresponding spec sections. It is the canonical worked example for the cohesive Authentication structure ([§10.3](/spec/next#103-authentication)) and the cross-flow scope composition rules ([§10.4.5](/protocol#1045-composition-across-boundaries-multi-hop-authorization)).
...
*Figure: One delegated envelope, reduced to the minimum scope at each hop, across three boundary types — human→agent (OAuth 2.1), agent→MCP (OAuth token exchange), and agent→agent (ADL passport + presentation proof). Required scopes are pulled from each counterparty's passport at connect time; every hop authenticates independently.*
...
- **Personal Assistant** has an Ed25519 keypair (private key in `assistant.example`'s KMS), a DID Document at `https://assistant.example/agents/personal-bot/did.json`, and a signed passport at `https://assistant.example/agents/personal-bot`. The passport's `cryptographic_identity.public_key.value` matches the DID Document's `assertionMethod` key (the [§10.3.1.4](/protocol#10314-public-key-cross-check) cross-check anchor).
...
**Authorization path:** [§10.4.3](/protocol#1043-authorization-in-human-to-agent-flows) (Human-to-Agent Flows)
...
**Authorization path:** [§10.4.3](/protocol#1043-authorization-in-human-to-agent-flows) (the MCP server is itself an OAuth 2.1 resource server)
...
If the calendar MCP did speak ADL, the assistant would use [§10.3.1](/protocol#1031-passport-verification-procedure) + [§10.3.2](/protocol#1032-presentation-proof) + [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) instead — the same machinery as Hops 4–6. The choice of authentication mechanism is a property of the *server*, not the *client*. ADL agents speak OAuth 2.1 to non-ADL servers and ADL passports to ADL servers, gracefully.
...
4. [§10.4.3](/protocol#1043-authorization-in-human-to-agent-flows) authorization at the MCP server:
...
**Authorization path:** None at retrieval. **[§10.3.1](/protocol#1031-passport-verification-procedure) verification** runs on each discovered passport before they enter the assistant's candidate list.
...
3. For each candidate, the assistant fetches the passport (HTTPS GET to `adl_document`) and runs the full [§10.3.1](/protocol#1031-passport-verification-procedure) verification procedure:

    - **[§10.3.1.1](/protocol#10311-retrieval-integrity) Retrieval Integrity** — TLS validation against the candidate's domain.
    - **[§10.3.1.2](/protocol#10312-schema-validation) Schema Validation** — passport conforms to the ADL schema.
    - **[§10.3.1.3](/protocol#10313-identity-resolution) Identity Resolution** — resolve `did:web:acme-flights.example:agents:booking` to its DID Document at `https://acme-flights.example/agents/booking/did.json`.
    - **[§10.3.1.4](/protocol#10314-public-key-cross-check) Public Key Cross-Check** — passport's inline `cryptographic_identity.public_key.value` matches the DID Document's `assertionMethod` key.
    - **[§10.3.1.5](/protocol#10315-signature-verification) Signature Verification** — the passport signature verifies against the resolved key.
    - **[§10.3.1.6](/protocol#10316-temporal-validity) Temporal Validity** — `security.attestation.expires_at` is well in the future.
    - **[§10.3.1.7](/protocol#10317-lifecycle-gating) Lifecycle Gating** — `lifecycle.status: "active"`. (Budget Air's status is `deprecated`, which the assistant logs but doesn't filter — it's still safe to use, just with a sunset warning. If it were `retired`, the assistant would skip it entirely.)
    - **[§10.3.1.8](/protocol#10318-provideridentity-coherence) Provider–Identity Coherence** — TLS authority `acme-flights.example` matches `provider.url` host matches the `did:web` domain segment.
    - **[§10.3.1.9](/protocol#10319-permission-and-classification-compatibility) Permission/Classification Compatibility** — the assistant is invoking the agent shortly, so it pre-checks classification: assistant's `data_classification.sensitivity` (let's say `internal`) is `≥` flight agent's classification (`internal`). ✓
...
- **Authentication path:** Acme is an ADL agent and exposes its `did:web` identity, so the assistant will authenticate agent-to-agent ([§10.3.1](/protocol#1031-passport-verification-procedure) + [§10.3.2](/protocol#1032-presentation-proof)) rather than via the human OAuth path. (A counterparty that only spoke OAuth would advertise only `security.authentication`, and the assistant would fall back to the [Hop 2](#hop-2-personal-assistant--calendar-mcp-server) pattern.)
...
- For Budget Air specifically: verified=true, severity=warn at [§10.3.1.7](/protocol#10317-lifecycle-gating)
...
**Authentication path:** [§10.3.1](/protocol#1031-passport-verification-procedure) (passport already verified in Hop 3, cached) + [§10.3.2](/protocol#1032-presentation-proof) (presentation proof, fresh per request)
**Authorization path:** [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) (Agent-to-Agent Flows)
...
This is the **reduction pattern** from [§10.4.5](/protocol#1045-composition-across-boundaries-multi-hop-authorization) applied per-counterparty: the requirement is pulled from the counterparty, not pushed by the assistant, and the assistant claims the minimum the counterparty asked for rather than the maximum Alice granted. For *this* search request, that minimum is `flights:search`.
...
The signature covers the JCS-canonical form of the proof minus the `signature` object (per [§10.3.2.6.5](/protocol#10326-verification-procedure)). Since `scopes` is part of the canonical bytes, an attacker who scrapes the proof cannot replay it with an expanded scope set.
...
1. **[§10.3.1](/protocol#1031-passport-verification-procedure) Passport Verification** — already cached from Hop 3 (or re-run fresh if cache expired). All 9 steps pass.

2. **[§10.3.2.6](/protocol#10326-verification-procedure) Proof Verification:**
    - **[§10.3.2.6.1](/protocol#10326-verification-procedure) Parsing** — proof is valid JSON, all required members present.
    - **[§10.3.2.6.2](/protocol#10326-verification-procedure) Issuer Match** — proof.iss `https://assistant.example/agents/personal-bot` equals passport.id ✓.
    - **[§10.3.2.6.3](/protocol#10326-verification-procedure) Temporal Validity** — current time is between `iat - 60s` and `exp + 60s`, and `exp - iat ≤ 5min` ✓.
    - **[§10.3.2.6.4](/protocol#10326-verification-procedure) Request Binding** — proof.request.method = `POST` matches actual method ✓; proof.request.uri canonicalizes to actual URI ✓.
    - **[§10.3.2.6.5](/protocol#10326-verification-procedure) Signature Verification** — Ed25519 signature verifies against JCS canonical bytes using the verification key from [§10.3.1.4](/protocol#10314-public-key-cross-check) ✓.
    - **[§10.3.2.6.6](/protocol#10326-verification-procedure) Replay Prevention** — `jti` not in the verifier's recent-cache ✓; insert it.
    - **[§10.3.2.6.7](/protocol#10326-verification-procedure) Nonce Verification** — N/A (no server-issued nonce required).

3. **[§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) Authorization (Agent-to-Agent):**
...
**Authentication path:** [§10.3.1](/protocol#1031-passport-verification-procedure) (cached) + [§10.3.2](/protocol#1032-presentation-proof) (fresh per request)
**Authorization path:** [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) with **escalated scopes**
...
[§10.3.1](/protocol#1031-passport-verification-procedure) cached. [§10.3.2.6](/protocol#10326-verification-procedure) runs fresh on the new proof — all checks pass.

[§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) authorization:
...
Same pattern as Hops 4 & 5, but against `https://luxury-hotels.example/agents/concierge` with `hotels:search` then `hotels:book + payments:authorize`. The cached [§10.3.1](/protocol#1031-passport-verification-procedure) verification of the hotel agent's passport reused; fresh [§10.3.2](/protocol#1032-presentation-proof) proof per request; [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) ceiling and tool checks identical in shape.
...
If the gap is only discovered mid-task (e.g., a counterparty requires a scope the assistant could not have anticipated from the intent), the assistant has two honest options: complete the part of the task it *is* authorized for and report what it could not do, or pause and request the additional grant. It does **not** silently claim authority it wasn't given — the upstream [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) ceiling check (next failure mode) is the backstop that makes sure of that. The design goal is that this is rare: front-loading the delegation to match the intent keeps the agent working autonomously to completion in the common case.

### The assistant's passport ceiling doesn't include `flights:book`

Maybe the assistant was provisioned without `flights:book` because the operator didn't enable that capability. At Hop 5, the assistant could *try* to issue the proof anyway, but the flight agent's [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) step 4 (ceiling subset check) would reject:
...
The flight agent rejects with a structured error pointing at [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) step 4. The assistant logs the misconfiguration and reports back to Alice that flight booking isn't enabled for this assistant.
...
The discovery document at Hop 3 listed Acme Flight Booking as `active`, but between Hop 3 and Hop 4 the agent was retired. When the assistant re-runs [§10.3.1](/protocol#1031-passport-verification-procedure) on the cached passport (or hits a still-cached signed passport whose lifecycle changed in the document at the canonical URL), [§10.3.1.7](/protocol#10317-lifecycle-gating) catches it:

- If the cached passport says `active` but the canonical URL now says `retired`, the [§10.3.1.3](/protocol#10313-identity-resolution) byte-sequence check catches the divergence (the old passport doesn't match the current canonical version).
- If the assistant re-fetches and gets the new `retired` passport, [§10.3.1.7](/protocol#10317-lifecycle-gating) hard-blocks.
...
[§10.3.2.6.4](/protocol#10326-verification-procedure) catches this: the replayed proof has `request.uri = .../search_flights` but the actual request URL is `.../book_flight`. URI mismatch → reject.

If the attacker also tries to replay the proof at the same URI (a duplicate search), [§10.3.2.6.6](/protocol#10326-verification-procedure) catches it: the `jti` is in the recent-cache → reject.

If the attacker has *forged* a proof for the right URI but signed with their own key, [§10.3.2.6.5](/protocol#10326-verification-procedure) catches it: the signature doesn't verify against the assistant's key.

If the attacker has the assistant's private key, none of this matters — the assistant has been compromised, and operational rotation (rotating the keypair, issuing a new attestation, marking the old as superseded via §5.6 lifecycle) is the answer. The presentation proof addresses replay; private key compromise is out of scope per [§10.3.2.1](/protocol#10321-threat-model).
...
| Authorize Alice's request to assistant | [§10.4.3](/protocol#1043-authorization-in-human-to-agent-flows) |
| Token exchange to MCP | [§10.3.3.1](/spec/next#10331-oauth-21-type-oauth2) + RFC 8693 |
| MCP tool authorization | [§10.4.3](/protocol#1043-authorization-in-human-to-agent-flows) |
| Discovery retrieval | [§6.4](/spec/next#64-discovery) |
| Per-candidate verification | [§10.3.1](/protocol#1031-passport-verification-procedure) (all 9 steps) |
| Passport caching | [§10.3.1](/protocol#1031-passport-verification-procedure) (implementation discretion; vector pack covers freshness) |
| Proof construction | [§10.3.2](/protocol#1032-presentation-proof) (build) |
| Proof signing | [§10.2](/spec/next#102-attestation) + [§10.3.2](/protocol#1032-presentation-proof) |
| Proof verification at upstream | [§10.3.2.6](/protocol#10326-verification-procedure) (all 7 sub-steps) |
| Ceiling check | [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) step 4 |
| Required-scope check | [§10.4.4](/protocol#1044-authorization-in-agent-to-agent-flows) step 6 |
| Cross-hop authorization independence | [§10.4.5](/protocol#1045-composition-across-boundaries-multi-hop-authorization) (independent vs. reduction patterns) |
| Audit recording | [§10.4.5](/protocol#1045-composition-across-boundaries-multi-hop-authorization) (multi-hop record requirement) |
