...
import governanceRegistryAgent from './examples/composite/governance-registry-agent.json';
...
Governance composes with other profiles via `allOf`. This composite example
declares both `urn:adl:profile:governance:1.0` and `urn:adl:profile:registry:1.0`,
combining governance members with the registry catalog members in a single
document.

<CodeTabs json={governanceRegistryAgent} title="composite/governance-registry-agent.adl" />
